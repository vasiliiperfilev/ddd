...
func TestNewDateDTO(t *testing.T) {
	...
	for _, c := range testCases {
		t.Run(c.Time.String(), func(t *testing.T) {
			dateDTO := infra.NewEmptyDateDTO(c.Time)
			assert.True(t, dateDTO.Date.Equal(c.ExpectedDateTime), "%s != %s", dateDTO.Date, c.ExpectedDateTime)
		})
	}
}

func newFirebaseRepository(t *testing.T, ctx context.Context) *infra.FirestoreHourRepository {
	firebaseClient, err := firestore.NewClient(ctx, os.Getenv("GCP_PROJECT"))
	require.NoError(t, err)

	repo := infra.NewFirestoreHourRepository(firebaseClient)
	return repo
}
...
func assertHourInRepository(ctx context.Context, t *testing.T, repo *infra.FirestoreHourRepository, hour *hour.Hour) {
